...
export default {
  ...
  async execute(args, context) {
    ...
    for (const item of slice) {
      const entry = esc(String(item.entry ?? ""));
      const ts = formatTimestamp(item.timestamp);
      const timeStr = esc(ts);
      ...
      items += `<div class="result-item"><div class="result-body"><div class="result-url-row"><span class="result-favicon result-favicon--clock">${CLOCK_ICON}</span><cite class="result-cite">${timeStr}</cite><a href="${esc(deleteUrl)}" class="history-delete-btn" aria-label="Delete">${TRASH_ICON}</a></div><a class="result-title" href="${esc(searchUrl)}">${entry}</a></div></div>`;
    }
