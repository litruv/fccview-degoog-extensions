...
function parseGitHubUrls(results) {
  const repos = new Map();
  const users = new Map();
...
export const slot = {
  ...
  async execute(_query, context) {
    ...
    const { repos, users } = parseGitHubUrls(results);
    ...
    for (const { owner, repo } of repos) {
      ...
      if (!data) continue;
      const fullName = esc(data.full_name || `${owner}/${repo}`);
      const desc = esc((data.description || "").slice(0, 160));
      const stars = formatCount(data.stargazers_count);
      const lang = esc(data.language || "");
      const href = esc(data.html_url || `https://github.com/${owner}/${repo}`);
      const avatarUrl = data.owner && data.owner.avatar_url ? esc(data.owner.avatar_url) : "";
      const avatarHtml = avatarUrl
        ? `<img src="${avatarUrl}" alt="" class="gh-slot-avatar-img" loading="lazy">`
        : `<span class="gh-slot-avatar-placeholder">${esc((owner || "?").charAt(0))}</span>`;
      repoCards.push(
        `<a href="${href}" target="_blank" rel="noopener" class="gh-slot-card gh-slot-repo">` +
    ...
    for (const login of users) {
      ...
      if (!data) continue;
      const name = esc(data.name || data.login);
      const bio = esc((data.bio || "").slice(0, 120));
      const href = esc(data.html_url || `https://github.com/${login}`);
      const avatarUrl = data.avatar_url ? esc(data.avatar_url) : "";
      const avatarHtml = avatarUrl
        ? `<img src="${avatarUrl}" alt="" class="gh-slot-avatar-img" loading="lazy">`
        : `<span class="gh-slot-avatar-placeholder">${esc((data.login || "?").charAt(0))}</span>`;
      const reposCount = formatCount(data.public_repos);
      const followersCount = formatCount(data.followers);
      userCards.push(
        `<a href="${href}" target="_blank" rel="noopener" class="gh-slot-card gh-slot-user">` +
        ...
        `<span class="gh-slot-user-login">@${esc(data.login)}</span>` +
        `</div>` +
        `</div>` +
    ...
    return { title: "GitHub", html: render({ content }) };
  },
};
